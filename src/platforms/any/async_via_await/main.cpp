...
auto get_image_buffer(const std::string URL, bool log = false) -> std::vector<std::string> {
  std::string site;
  std::string page;
...
auto async_fn(const std::string URL) -> std::future<std::vector<std::string>> {
  return std::async(std::launch::async, get_image_buffer, URL, false);
}

int main(int argc, char *argv[]) {
  stop_watch.reset();
  auto future = async_fn("http://www.korea.edu/dext5editordata/2020/12/20201210_093034568_30817.png");
  ...
  std::ofstream file;
  file.open("image_a_a.png", std::ios::out | std::ios::binary);
